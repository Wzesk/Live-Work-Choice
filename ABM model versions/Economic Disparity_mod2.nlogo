...
to go
  ...
  let pten mean [tenure] of poor
  let rten mean [tenure] of rich
  set average-tenure (ticks - ( ( pten + rten ) / 2 ) )

  let page mean [birth] of poor
  let rage mean [birth] of rich
  set average-age (ticks - ( ( page + rage ) / 2 ) )
...
to apply-age-color
  ...
  ask poor
  [
    set color scale-color blue price 0 100
    set size 3
  ]
    ask rich
  [
...
to-report weibull [ wx ]
  let gamma 2
  let wy ( ( gamma / 2 ) * ( ( wx * 3 ) ^ ( gamma - 1 ) ) ) * (exp ( - ( ( wx * 3 ) ^ gamma ) ) )
  report wy
end
